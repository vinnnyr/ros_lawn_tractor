...
void steer()
{
    ...
    // steer smoothing using SimpleKalmanFilter
...
void timerIsr()
{
  ...
  // Speed is calculated by the formula for circumference divided by the number of ticks
  // This gives us the distance travelled.  We have timer set to calculate the distance
  // Every .2 seconds.  We take:
  //    ((2*Pi*Tire_Radius) / (Number of Ticks Counted)) / (Time we counted ticks)
